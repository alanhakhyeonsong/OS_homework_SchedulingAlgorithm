...
class Process {
	...
	Process(String name, int arrivalTime, int serviceTime) { // 프로세스의 각 필드를 초기화함
		/* 각 멤버를 여기서 초기화 할 것 */
		this.name = name; this.arrivalTime = arrivalTime; this.serviceTime = serviceTime;
		executionTime = 0;
	}
	/* 아래 각 함수를 구현할 것 */
	public void	   incExecTime() 			{ executionTime++; } /* 매 시간단위마다 현재 실행 중인 프로세스의 실행시간을 증가시킴 */
	public int     getServiceTime()  		{ return serviceTime; } /* 서비스시간을 얻어옴 */
	public int     getWaitingTime(int cTime){ return (cTime - arrivalTime); } /* 레디큐에 도출한 후 지금(cTime)까지의 대기시간을 계산해 옴, cTime은 현재시간임, getResponeRatioTime()에서 호출함 */
	public int     getRemainingTime()		{ return (serviceTime - executionTime); } /* 남을 실행시간을 계산해 옴 */
	public double  getResponeRatioTime(int cTime)  { return (double)((getWaitingTime(cTime) + serviceTime) / serviceTime); } /* 프로세스의 응답비율(Response Ratio)를 계산해 옴, cTime은 현재시간임 */
...
//SRT에 schedule 함수틀 그대로 쓰고 for문을 알고리즘에 맞게 바꾸면 됨.
// isSchedulable 함수를 넣을지 말지는 선점형, 비선점형에 따라 다르니까 이 또한 알고리즘에 맞게 넣으면 됨.

// Shortest Process Next 알고리즘을 구현하였습니다.
// SRT의 비선점형 버전으로 각 프로그램이 종료된 후 스케줄링합니다. 따라서 isSchedulable() 함수를 재정의할 필요가 없습니다.
// for문은 다음과 같이 레디큐의 요소들(프로세스들) 하나하나마다 멤버 getServiceTime() 메소드를 통해 프로세스의 실행 시간을 불러와 비교하여 가장 짧은 것을 다음 실행할 프로세스로 스케줄링하도록 구현했습니다.
...
// Highest Response Ratio Next 알고리즘을 구현하였습니다.
// Response Ratio가 가장 높은 프로세스가 우선순위가 가장 높으며, 비선점형 알고리즘으로 각 프로세스가 종료된 후 스케줄링 합니다.
// 따라서 isSchedulable() 함수를 재정의할 필요가 없습니다.
// for문은 다음과 같이 레디큐의 요소들(프로세스들) 하나하나마다 멤버 getResponeRatioTime() 메소드를 통해 응답비율을 불러와 비교하여 가장 높은 것을 다음 실행할 프로세스로 스케줄링하도록 구현했습니다.
// getResponeRatioTime()메소드의 인자는 현재 시간을 불러와야 하므로 currentTime입니다.
// clockInterrupt() 메소드를 고려하면 함수 내에서 매 시간마다 currentTime을 1씩 증가 시키고 있기 때문에 이를 불러오는 것이 맞다고 생각합니다.
...
public class SchedulingApp_20174329 {
